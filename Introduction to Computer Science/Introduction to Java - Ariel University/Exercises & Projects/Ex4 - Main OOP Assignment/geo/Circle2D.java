...
public class Circle2D implements GeoShapeable{
	...
	public Point2D[] getPoints() {
		// we return all the "corner" points - to calc bounding box later
		// we use only the first one for the drawing
		Point2D[] ans = new Point2D[5];
		ans[0] =new Point2D(this._center);
		ans[1] = new Point2D(ans[0].x(), ans[0].y()+this._radius);
		ans[2] = new Point2D(ans[0].x()-this._radius, ans[0].y());
		ans[3] = new Point2D(ans[0].x(), ans[0].y()-this._radius);
		ans[4] = new Point2D(ans[0].x()+this._radius, ans[0].y());
		return ans;
	}
	@Override
	public void scale(Point2D center, double ratio) {
		// Multiply the radius by the required ratio
		_radius *= ratio;
