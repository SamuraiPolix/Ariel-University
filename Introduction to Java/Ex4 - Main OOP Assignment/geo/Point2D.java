...
public class Point2D {
	...
	public void scale(Point2D cen, double ratio) {
		// we calculate the value that needs to be added for each point and add it
		// figured it out by seeing how your program performed
		// we made a shape, save it to file, changed to int values,
		// loaded it, scaled, saved and saw the change that happened-
		// which gave us indication.
		double dx, dy;
		dx = (cen.x()-_x);
		dy = (cen.y()-_y);
		_x += (dx*(1-ratio));
	...
	public void rotate(Point2D cen, double angleDegrees) {
		// Calculate the value required to add to the curr point:
		// dif in x * (1-ratio) and the same with y
		double dx, dy;
		dx = _x - cen.x();
		dy = _y - cen.y();

		_x = (Math.cos(angleDegrees) * dx) - (Math.sin(angleDegrees) * dy) + cen.x();
		_y = (Math.sin(angleDegrees) * dx) + (Math.cos(angleDegrees) * dy) + cen.y();
	}
